...
// 1hc substitution of src/linux-define.h
...
# define INPUT_STRING(str) do {                                                 \
        cut_Input(strlen(str), str);                                            \
    } while (0)

# define INPUT_BYTES(array) do {                                                \
...
int cut_Input(size_t size, const char *bytes);
...
#  if defined(__linux__)
// 1hc substitution of src/linux.h
#ifndef CUT_LINUX_H
#define CUT_LINUX_H
...
// 1hc substitution of src/unix-common.h
...
CUT_PRIVATE void cut_InputProcess(int write_fd, size_t size, const char *bytes) {
    // silence SIGPIPE, we will detect it in write()
    signal(SIGPIPE, SIG_IGN);

    size_t cursor = 0;
    while (cursor < size) {
        ssize_t written = cut_Write(write_fd, bytes + cursor, size - cursor);
...
CUT_PRIVATE void cut_ReapInputFeed(void) {
    ...
    if (kill(cut_inputFeedPID, SIGKILL) == -1) {
...
int cut_Input(size_t size, const char *bytes) {
    ...
    if (pid == 0) {
        ...
        cut_SetPDeathSignal(parentPid);
        close(pipefd[0]);
        cut_InputProcess(pipefd[1], size, bytes);
        exit(cut_NORMAL_EXIT);
    }
    ...
    // std* symbols cannot be (portably) assigned to,
    // so freopen() must be used here
